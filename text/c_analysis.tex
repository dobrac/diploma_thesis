...
\begin{itemize}
    ...
    \item generation from live endpoint and .graphql definition files,
...
\begin{itemize}
    ...
    \item not actively maintained (last commit 3 years ago)~\cite{graphql-graphql-playground}.
\end{itemize}

\subsubsection{GraphQL Playground}
GraphQL Playground is an application for desktop and web with the ability of calling GraphQL services.
It allows developers to build and test GraphQL queries and mutations, explore an APIs schema, and view real-time results.
...
It uses live endpoint and can be hosted as static website.
...
\begin{itemize}
    ...
    \item not actively maintained (last commit 2 years ago, last release in 2019)~\cite{graphql-graphql-playground}.
\end{itemize}

\subsubsection{GraphiQL}
GraphiQL is a interactive in-browser IDE\@.
It allows GraphQL API documentation exploring as well as the ability to execute queries and mutations.
...
One of the most popular one is Swagger UI, which I will mainly cover and compare the gRPC options to.

\subsubsection{ReDoc}
ReDoc is a tool for generating documentation from OpenAPI definition.
...
It is a static website, supporting documentation comments, and allows calling the endpoints.
...
Swagger UI is one of the most popular choice for RESTful API documentation.
The parent project is OpenAPI, which is a specification for building HTTP APIs in general.
This tool is used to generate documentation from OpenAPI definition.
It is a static website with support for comments and interactive endpoints calling.
